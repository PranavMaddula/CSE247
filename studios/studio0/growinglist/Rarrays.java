...
/**
 * Measure the time (ticks) needed to replace a full array with a new, bigger
 * one. How much bigger should the new array be? You examine several strategies
 * for sizing the new, bigger array and you study the effects of those
 * strategies on overall execution time (ticks).
 * 
 * @author roncytron
 *
 */
abstract public class Rarrays extends QuietAlgorithm {
	...
	/**
	 * As a "quiet" algorithm, all we care about is the size parameter. So we keep
	 * filling the current array until we have a total of size elements. But along
	 * the way, the array gets full. When it does, we replace the old array with a
	 * bigger one.
	 */
	@Override
	public void run() {
	...
	/**
	 * This is abstract, so that extending classes can try their own strategies
	 * about how much larger the new array should be.
	 * 
