...
public class LinkedListWithTail<T> implements List<T> {
	...
	/**
	 * Modify this method for studio 2 so that it uses an instance variable for the
	 * tail reference, and modify code in this class so that the tail reference that
	 * always points to the end of the list.
	 * 
	 * Use that tail reference to add to the end of the list, instead of looping to
	 * find the end.
	 */
	@Override
	public void addLast(T thing) {
		if (head == null) {
			...
			ticker.tick(5); // for the 5 statements above + if statement
		} else {
...
//			p.next = q;
	...
	/**
	 * Modify this method so that getting the size of this list takes Theta(1)
	 * (constant) time.
	 */
	@Override
	public int getSize() {
